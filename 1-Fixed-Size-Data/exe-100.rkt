...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname exe-100) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp")) #f)))
...
(check-expect (si-game-over (make-aim (make-posn 10 (- HEIGHT (image-height UFO-IMAGE))) (make-tank 38 -3))) true)
(check-expect (si-game-over (make-aim (make-posn 10 (- 199 (image-height UFO-IMAGE))) (make-tank 38 -3))) false)

(check-expect (si-game-over (make-fired (make-posn 10 (- HEIGHT (image-height UFO-IMAGE))) (make-tank 38 -3) (make-posn 99 99))) true)
(check-expect (si-game-over (make-fired (make-posn 10 (- 199 (image-height UFO-IMAGE))) (make-tank 38 -3) (make-posn 99 99))) false)

(check-expect
 (si-game-over (make-fired (make-posn 10 20) (make-tank 38 -3) (make-posn (+ 10 (/ (image-width UFO-IMAGE) 2)) (+ 20 (/ (image-height UFO-IMAGE) 2)))))
 true)
(check-expect
 (si-game-over (make-fired (make-posn 10 20) (make-tank 38 -3) (make-posn (+ 11 (/ (image-width UFO-IMAGE) 2)) (+ 20 (/ (image-height UFO-IMAGE) 2)))))
 false)
(check-expect
 (si-game-over (make-fired (make-posn 10 20) (make-tank 38 -3) (make-posn (+ 10 (/ (image-width UFO-IMAGE) 2)) (+ 21 (/ (image-height UFO-IMAGE) 2)))))
 false)
...
(define (si-render-final s)
  (overlay
   (text
    (if (ufo-landed? (if (aim? s) (aim-ufo s) (fired-ufo s))) "GAME OVER" "YOU WIN")
    26
    "blue") CANVAS)
  )
...
(si-main (make-aim (make-posn (/ HEIGHT 2) 0) (make-tank (/ HEIGHT 2) TANK-SPEED)))